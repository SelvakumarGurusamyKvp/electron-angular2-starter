...
ipcMain.on('opened-with-file', (event, arg) => {
  // if(process.argv.length > 0){
  //   ipcMain.emit('opened-with-file', process.argv);
  // }
  // event.sender.send('opened-with-file', process.argv);
...
function onOpen(event, args){
  setTimeout(() => {
    devToolsLog("———————————————", "");
    devToolsLog("EVENT - ", event);
    devToolsLog("ARGS - ", args);
    devToolsLog("ARGV - ", process.argv);
    // ipcMain.emit('opened-with-file', {event: event, args: args, argv: process.argv});
  }, 4000)
}

function devToolsLog(s, arg) {
  arg = JSON.stringify(arg);
  console.log(s, arg);
  if (mainWindow && mainWindow.webContents) {
    mainWindow.webContents.executeJavaScript(`console.log('${s}, ${arg}')`)
  }
}
